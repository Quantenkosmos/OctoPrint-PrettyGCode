...
//rate=Math.min(rate,window.myMaxRate);
        ...
        self.onAfterBinding = function () {
            // console.log("PrettyGCode: onAfterBinding"); // Removed debug log
            
            // Try attaching context menu blocker early - Removed, moving to initThree
            // $("#mycanvas").on('contextmenu', function(e){ 
            //     console.log("PrettyGCode: Canvas context menu blocked (early)");
            //     return false; 
            // });
        ...
        self.onTabChange = function (current, previous) {
            console.log("PrettyGCode: Tab changed to", current);
                ...
                if (!viewInitialized) {
                    ...
                    self.printerProfiles.currentProfileData.subscribe(
                        function(){
                            ...
                            try {
                                updateBedVolume();
                            } catch(e) { console.error("Error updating bed volume:", e); }
                    ...
                    try {
                        updateBedVolume();
                    } catch(e) { console.error("Error initial bed volume update:", e); }
...
//                                syncLayerToZ();
                    ...
                    $("#myslider").css("pointer-events", "auto"); // Re-enable pointer events for the slider

                    // Re-enable pointer events for buttons and mygui
                    $("#mygui").css("pointer-events", "auto");
                    $(".pgstatetoggle, .pgfilestoggle, .pgsettingstoggle, .fstoggle, .pgdashtoggle, .pgcameratoggle").css("pointer-events", "auto");
                ...
                // self.controlViewModel._enableWebcam(); // Removed problematic call
                ...
                // self.controlViewModel._disableWebcam(); // Removed problematic call
            }
            // self.controlViewModel._enableWebcam(); // Removed problematic call
        };
...
//                        per= Math.abs(vec.dot(new THREE.Vector3(1,0,0)),0.0)
        ...
        function initThree()
        {
            console.log("PrettyGCode: initThree starting");
            var canvas = $("#mycanvas");
            
            if (canvas.length === 0) {
                console.error("PrettyGCode: Canvas #mycanvas not found!");
            } else {
                console.log("PrettyGCode: Canvas found", canvas[0]);
                
                //Prevent context menu so right click works for panning
                canvas.on('contextmenu', function(e){ 
                    console.log("PrettyGCode: Canvas context menu blocked");
                    return false; 
                });
            }
            ...
            try {
                ...
                console.error("Error initializing CameraControls:", e);
            }
